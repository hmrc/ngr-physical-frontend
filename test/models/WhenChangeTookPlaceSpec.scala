...
class WhenChangeTookPlaceSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {
  ...
  "WhenChangeTookPlace" - {
    ...
    "must fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!WhenChangeTookPlace.values.map(_.toString).contains(_))
