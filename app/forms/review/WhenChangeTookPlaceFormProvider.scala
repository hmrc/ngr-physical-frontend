...
import models.review.WhenChangeTookPlace
...
object WhenChangeTookPlaceData {
  implicit val format: OFormat[WhenChangeTookPlaceData] = Json.format
}

class WhenChangeTookPlaceFormProvider @Inject() extends Mappings {

  def apply()(implicit messages: Messages): Form[WhenChangeTookPlaceData] =
    Form(
      mapping(
